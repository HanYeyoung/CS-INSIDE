...
def calculate_similarity(course_name):
    ...
    data = [
        {"name": c.name, "avg_grades": c.avg_grades, "total_grades": c.total_grades, "avg_gpa": c.avg_gpa}
        for c in courses
    ]
    ...
    if df.isnull().values.any():
        df = df.dropna(subset=["avg_grades", "total_grades", "avg_gpa"])
        if df.empty:
            return None, "All course data is incomplete or invalid."

    # Normalize course_name to ensure case-insensitive matching
